...
/*
 * 1 = relay activates with HIGH
 * 0 = relay activates with LOW
 */
...
".wrap{width:min(680px,100%);margin:auto;padding:18px 14px 34px}"
...
"<header class='top'>"
"<h1>Smart Home</h1>"
"<div class='sub'>Local offline control</div>"
"</header>"
...
"<section class='card'>"
"<div class='row'>"
"<div><div class='name'>Firmware</div>"
"<div class='state'>Update locally from this device</div></div>"
"<button onclick='show(\"ota\")'>OTA Update</button>"
"</div>"

"<div id='ota' class='hidden'>"
"<label class='field'>Firmware .bin</label>"
"<input id='fw' type='file' accept='.bin,application/octet-stream'>"
"<div class='bar'>"
"<button class='primary' onclick='uploadFirmware()'>Upload & Restart</button>"
"</div>"
...
"<section class='card'>"
"<div class='row'>"
"<div><div class='name'>Access Point</div>"
"<div class='state'>ESP32 local network settings</div></div>"
...
"<label class='field'>Password (8-63 characters)</label>"
...
"let h='';"
"a.forEach((v,i)=>{"
"h+=`<section class='card'><div class='row'>"
"<div><div class='name'>${names[i]}</div>"
"<div class='state' id='st${i}'>${v?'ON':'OFF'}</div></div>"
"<label class='switch'>"
"<input type='checkbox' id='r${i}' ${v?'checked':''} "
"onchange='setRelay(${i},this.checked)'>"
...
"if(!r.ok)throw 0;"
"let d=await r.json();"
"render([!!d.relay1,!!d.relay2,!!d.relay3]);"
"}catch(e){"
...
"el.disabled=true;"
"try{"
"let r=await fetch(`/api/relay?relay=${i+1}&state=${on?1:0}`,{"
"cache:'no-store'"
"});"
"if(!r.ok)throw 0;"
"await load();"
...
"let m=document.getElementById('setmsg');"

"if(s.length<1||s.length>32||p.length<8||p.length>63){"
"m.textContent='Invalid SSID or password.';"
"return;"
"}"
...
"}catch(e){"
"m.textContent='Connection lost. The AP may be restarting.';"
"}"
"}"

"async function loadSettings(){"
"try{"
"let r=await fetch('/api/settings',{cache:'no-store'});"
"let d=await r.json();"
...
"let m=document.getElementById('otamsg');"
...
"if(f.size<1024){"
"m.textContent='Firmware file is too small.';"
"return;"
"}"

"if(!confirm('Start OTA update? The device will restart after a successful update.'))"
"return;"

"m.textContent='Uploading... Do not disconnect.';"

"try{"
"let r=await fetch('/api/ota',{"
"method:'POST',"
...
"}catch(e){"
"m.textContent='Upload interrupted. Current firmware should remain active.';"
"}"
"}"
...
static void load_nvs(void)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGW(TAG, "No existing configuration; using defaults");
        return;
    }
 ...
 *
 * IMPORTANT:
 * Storage mutex is acquired BEFORE taking the relay snapshot.
 * This prevents two rapid relay changes from writing an older
 * snapshot after a newer snapshot.
 * ============================================================ */
...
static esp_err_t save_ap_settings(
    const char *ssid,
    const char *password
)
{
...
static void apply_all_relays(void)
{
    ...
    gpio_set_level(
        RELAY1_GPIO,
        relay_output_level(s[0])
    );

    gpio_set_level(
        RELAY2_GPIO,
        relay_output_level(s[1])
    );

    gpio_set_level(
        RELAY3_GPIO,
        relay_output_level(s[2])
    );
}
...
static void init_relays(void)
{
    ...
    /*
     * Physical OFF first.
     * Persistent states are applied immediately afterward.
     */
...
static void wifi_init_ap(void)
{
    ...
    /*
     * ESP-IDF 5.x compatible IP configuration.
     *
     * Avoid ip4addr_aton() type mismatch by assigning
     * using IP4_ADDR().
     */
    ...
    ap.ap.pmf_cfg.required =
        false;

    ap.ap.pmf_cfg.capable =
        true;
...
)
{
    ...
    out[2] =
        (uint8_t)(query[2] | 0x80);

    out[3] =
        (uint8_t)(query[3] & 0xF0);
...
static esp_err_t redirect_to_root(
    httpd_req_t *req
)
{
    httpd_resp_set_status(
...
static esp_err_t relay_handler(
    httpd_req_t *req
)
{
    if (ota_in_progress) {
    ...
    /*
     * Apply hardware and state atomically
     * with respect to other relay operations.
     */
     ...
     * Persistent save.
     *
     * If NVS fails, hardware state remains applied.
     */
    ...
    if (err != ESP_OK) {
        ...
        return send_json(
            req,
            "{\"error\":\"state applied but not saved\"}",
...
static esp_err_t settings_get_handler(
    httpd_req_t *req
)
{
    /*
     * Password intentionally isn't returned to the browser.
     */
...
)
{
    ...
    while (
        ...
        *p != '"' &&
        ...
        /*
         * JSON escapes are intentionally rejected.
         * This keeps the parser deterministic and small.
         */
...
static esp_err_t settings_post_handler(
    httpd_req_t *req
)
{
    if (ota_in_progress) {
    ...
    /*
     * Allow response to leave the socket before restart.
     */
...
)
{
    ...
    /*
     * This validates the written image.
     */
    ...
    ESP_LOGI(
        TAG,
        "OTA image validated and boot partition updated"
    );
    ...
    /*
     * Give TCP response time to leave.
     */
...
static esp_err_t captive_handler(
    httpd_req_t *req
)
{
...
static void start_http_server(void)
{
    ...
    httpd_uri_t c5 = {
        .uri = "/connectivitycheck.gstatic.com/generate_204",
        .method = HTTP_GET,
        .handler = captive_handler
    };
...
static void watchdog_keepalive_task(
    void *arg
)
{
    /*
...
void app_main(void)
{
    ...
    if (
        ...
        /*
         * IMPORTANT:
         * Capture the result of the second initialization.
         */
    ...
    /*
     * Restore persistent relay state.
     */
